...
function ServiceCard({ href, title, description, emoji, locked, lockMsg }: {
  ...
  if (locked) {
    return (
      <div className="block rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 text-right opacity-60 relative">
        <div className="absolute top-3 left-3 text-xs bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2 py-0.5 rounded-full">🔒 مقفل</div>
        ...
        {lockMsg && <div className="mt-3 text-[11px] text-amber-400">{lockMsg}</div>}
      </div>
    );
  }
  return (
...
export default async function DashboardPage() {
  ...
  const dbUser = await prisma.user.findUnique({
    ...
    select: {
      ...
      _count: { select: { consultations: true, cases: true } },
    },
  });
  ...
  const effectivePlan  = isExpired ? "FREE" as UserPlan : plan;
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-50" dir="rtl">
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <header className="text-right space-y-1">
          <h1 className="text-2xl font-bold">مرحباً، {dbUser.name || user.email} 👋</h1>
          <p className="text-sm text-zinc-400">لوحة التحكم الخاصة بك في منصة المستشار القانوني الذكي.</p>
        </header>

        <PointsCard points={dbUser.points} plan={plan} subscriptionEndsAt={dbUser.subscriptionEndsAt?.toISOString() ?? null} />
        ...
        <section className="space-y-3">
          <h2 className="text-base font-semibold text-right text-zinc-300">خدماتك المتاحة</h2>
          <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            <ServiceCard href="/consultations" emoji="⚖️" title="الاستشارات القانونية" description="استشارة فورية بالذكاء الاصطناعي أو من محامٍ بشري أو مكتب معتمد." />
            <ServiceCard href="/contracts" emoji="📄" title="العقود الذكية" description="إنشاء عقود قانونية ذكية وتحميلها بصيغة PDF." locked={!effectivePerms.contracts} lockMsg={lockMsg} />
            <ServiceCard href="/translate" emoji="🌐" title="الترجمة القانونية" description="ترجمة قانونية احترافية بين العربية والإنجليزية." locked={!effectivePerms.humanTranslation && !effectivePerms.aiTranslation} lockMsg={lockMsg} />
            <ServiceCard href="/library" emoji="📚" title="المكتبة القانونية" description="الاطلاع على النصوص القانونية والدراسات الأكاديمية." />
            <ServiceCard href="/cases" emoji="📁" title="إدارة القضايا" description="متابعة قضاياك القانونية وجلسات المحكمة والمستندات." locked={!effectivePerms.caseManagement} lockMsg={lockMsg} />
            <ServiceCard href="/lawyers" emoji="🏛️" title="المحامون والمكاتب" description="تصفح المحامين المعتمدين والمكاتب القانونية." locked={!effectivePerms.viewLawyers && !effectivePerms.humanConsult} lockMsg={lockMsg} />
            <ServiceCard href="/smart-lawyer" emoji="🤖" title="المحامي الذكي" description="تحليل القضايا والمستندات بالذكاء الاصطناعي — باقة الشركات فقط." locked={!effectivePerms.smartLawyer} lockMsg="متاح لباقة الشركات فقط" />
          </div>
        </section>

        {effectivePlan === "FREE" && (
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 text-right">
            <div className="text-sm font-semibold text-amber-300 mb-1">🚀 افتح كامل إمكانيات المنصة</div>
            <p className="text-xs text-zinc-400 mb-3">أنت على الباقة المجانية — ترقيتك إلى باقة الأفراد تمنحك 50 نقطة شهرياً والوصول لكل الخدمات.</p>
            <Link href="/pricing" className="inline-block px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition">عرض الباقات ↗</Link>
          </div>
        )}
