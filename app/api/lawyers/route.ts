...
export async function GET(req: Request) {
  try {
    ...
    const where: any = {
      ...
      isApproved: true,
      branchId: null, // استثناء موظفي المكاتب المعتمدة
    };
    ...
    return NextResponse.json({ error: "حدث خطأ أثناء جلب قائمة المحامين." }, { status: 500 });
  }
}
  ...
  try {
    ...
    const created = await prisma.user.create({
      data: {
        ...
        role: UserRole.LAWYER,
        isApproved: false,
        branchId: null, // محامٍ فرد دائماً
        lawyerProfile: {
          create: {
    ...
    return NextResponse.json({ error: e.message || "حدث خطأ أثناء إنشاء المحامي." }, { status: 500 });
  }
}