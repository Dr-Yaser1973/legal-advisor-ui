// lib/contracts/om/agency.ts
// عقد وكالة وفق قانون المعاملات المدنية العُماني (المرسوم السلطاني رقم 29/2013) — أحكام الوكالة،
// ولوكالات التوزيع والاستيراد التجارية يُراعى قانون الوكالات التجارية الصادر بالمرسوم السلطاني رقم 26/77.
...
export const AGENCY_OM_AR: ContractTemplate = {
  id: 17601,
  slug: "om-agency-ar",
  title: "عقد وكالة (عُمان) – عربي",
  lang: "ar",
  group: "PRO",
  jurisdiction: "OM",
  fields: [
    ...
    { key: "principalId", label: "الرقم المدني/السجل للموكِّل", required: true, type: "text", group: "الموكِّل" },
    ...
    { key: "agentId", label: "الرقم المدني/السجل للوكيل", required: true, type: "text", group: "الوكيل" },
...
<div class="doc rtl">
  <div class="header">
    ...
    <div class="subtitle">مصاغ وفق قانون المعاملات المدنية العُماني (المرسوم السلطاني رقم 29/2013) — أحكام الوكالة</div>
    <div class="meta">
  ...
  <div class="box">
    ...
    <div class="p"><b>الموكِّل:</b> {{principalName}} — الرقم المدني/السجل: {{principalId}} — العنوان: {{principalAddress}}</div>
    <div class="p"><b>الوكيل:</b> {{agentName}} — الرقم المدني/السجل: {{agentId}} — العنوان: {{agentAddress}}</div>
  </div>
  ...
  <div class="box">
    ...
    <div class="clause">الوكالة العامة لا تخوّل الوكيل إلا أعمال الإدارة. أما البيع والرهن والتبرع والصلح والإقرار وسائر التصرفات التي تخرج عن الإدارة فلا بدّ فيها من تفويض خاص صريح، عملاً بأحكام الوكالة في قانون المعاملات المدنية.</div>
  ...
  <div class="box">
    ...
    <ol class="ol">
      ...
      <li>تنتهي الوكالة بإتمام العمل أو بانقضاء المدة أو بموت أحد الطرفين أو بعزل الوكيل أو باعتزاله، مع مراعاة المسؤولية عن العزل أو الاعتزال في وقت غير مناسب.</li>
    </ol>
  </div>
  ...
  <div class="box">
    ...
    <ol class="ol">
      <li>يخضع هذا العقد ويُفسَّر وفق: <b>{{governingLaw}}</b> (ولوكالات التوزيع والاستيراد التجارية يُراعى قانون الوكالات التجارية رقم 26/77 وقيدها لدى وزارة التجارة والصناعة وترويج الاستثمار).</li>
...
export const AGENCY_OM_EN: ContractTemplate = {
  id: 17602,
  slug: "om-agency-en",
  title: "Agency / Mandate Agreement (Oman) — English",
  lang: "en",
  group: "PRO",
  jurisdiction: "OM",
  fields: [
    ...
    { key: "principalId", label: "Principal Civil ID / CR", required: true, type: "text", group: "Principal" },
    ...
    { key: "agentId", label: "Agent Civil ID / CR", required: true, type: "text", group: "Agent" },
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="muted">Governed by the Omani Civil Transactions Law (Royal Decree No. 29/2013) — Mandate provisions.</div>
    </div>
    <div class="meta">
  ...
  <div class="box"><div class="sec"><h3>1. Parties</h3>
    <div><b>Principal:</b> {{principalName}} — Civil ID/CR: {{principalId}} — {{principalAddress}}</div>
    <div><b>Agent:</b> {{agentName}} — Civil ID/CR: {{agentId}} — {{agentAddress}}</div>
  </div></div>

  <div class="sec"><h3>2. Scope</h3><div class="box">
    <div><b>Scope:</b> {{scope}}</div>
    <div><b>Type:</b> {{agencyType}} &nbsp; <b>Sub-delegation:</b> {{subAgent}}</div>
    <div class="muted">A general mandate covers only acts of administration; sale, mortgage, donation, settlement, admission and other acts beyond administration require express special authority under the Civil Transactions Law.</div>
  ...
  <div class="sec"><h3>4. Agent's Duties</h3><div class="box">
    <div class="muted">The Agent shall act within, and not exceed, the scope of the mandate, exercise the care of an ordinary person, remain liable for fraud and gross fault, render account to the Principal and hand over whatever is received on the Principal's behalf.</div>
  </div></div>

  <div class="sec"><h3>5. Principal's Duties & Termination</h3><div class="box">
    <div class="muted">The Principal pays the agreed fee and reimburses expenses. The mandate ends on completion, expiry, death of a party, revocation or the agent's resignation, subject to liability for an untimely revocation or resignation.</div>
  </div></div>
  ...
  <div class="sec"><h3>6. Governing Law & Special Terms</h3><div class="box">
    ...
    <div class="muted">Commercial distribution and import agencies are additionally subject to the Commercial Agencies Law (Royal Decree No. 26/77) and its registration with the Ministry of Commerce, Industry and Investment Promotion.</div>
  </div></div>
