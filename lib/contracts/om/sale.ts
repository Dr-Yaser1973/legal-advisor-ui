// lib/contracts/om/sale.ts
// عقد بيع منقول وفق قانون المعاملات المدنية العُماني (المرسوم السلطاني رقم 29/2013) —
// أحكام البيع، وضمان الاستحقاق والتعرّض، وضمان العيوب الخفية.
...
const OM = getJurisdiction("OM");

export const SALE_OM_AR: ContractTemplate = {
  id: 17101,
  slug: "om-sale-movable-ar",
  title: "عقد بيع منقول (عُمان) – عربي",
  lang: "ar",
  group: "PRO",
  jurisdiction: "OM",
  fields: [
    ...
    { key: "contractCity", label: "مدينة الإبرام", required: true, type: "text", group: "معلومات العقد", placeholder: "مسقط" },

    { key: "sellerName", label: "اسم البائع", required: true, type: "text", group: "البائع" },
    { key: "sellerId", label: "الرقم المدني/السجل التجاري للبائع", required: true, type: "text", group: "البائع" },
    ...
    { key: "buyerId", label: "الرقم المدني/السجل التجاري للمشتري", required: true, type: "text", group: "المشتري" },
    ...
    { key: "priceCurrency", label: "العملة", required: true, type: "select", group: "الثمن والسداد",
      options: currencyOptionsAr(OM.currencies) },
    ...
    { key: "paymentMethod", label: "طريقة السداد", required: true, type: "select", group: "الثمن والسداد",
      options: ["نقداً", "حوالة بنكية", "شيك", "أقساط"] },
    ...
    { key: "governingLaw", label: "القانون الواجب التطبيق", required: true, type: "text", group: "أحكام", placeholder: OM.governingLawAr },
    { key: "disputeCity", label: "الاختصاص المكاني (المحكمة)", required: true, type: "text", group: "أحكام", placeholder: OM.defaultCourtCityAr },
...
<div class="doc rtl">
  <div class="header">
    ...
    <div class="subtitle">مصاغ وفق قانون المعاملات المدنية العُماني (المرسوم السلطاني رقم 29/2013)</div>
    <div class="meta">
  ...
  <div class="box">
    ...
    <table class="tbl">
      <tr><td class="th">البائع</td><td><b>الاسم:</b> {{sellerName}} — <b>الرقم المدني/السجل:</b> {{sellerId}} — <b>العنوان:</b> {{sellerAddress}} — <b>الهاتف:</b> {{sellerPhone}}</td></tr>
      <tr><td class="th">المشتري</td><td><b>الاسم:</b> {{buyerName}} — <b>الرقم المدني/السجل:</b> {{buyerId}} — <b>العنوان:</b> {{buyerAddress}} — <b>الهاتف:</b> {{buyerPhone}}</td></tr>
    </table>
  ...
  <div class="box">
    ...
    <div class="clause">يلتزم المشتري بأداء الثمن في المكان والزمان المتفق عليهما، ويقرّ البائع باستلامه بحسب ما يثبت بالإيصالات عند الاقتضاء، ويُعدّ العقد ملزماً للطرفين فلا ينفرد أحدهما بنقضه أو تعديله إلا باتفاقهما أو بما يقرّره القانون.</div>
  </div>
  ...
  <div class="box">
    ...
    <ol class="ol">
      ...
      <li>تنتقل ملكية المنقول إلى المشتري ما لم يُتفق على غير ذلك، وينتقل هلاك المبيع وتبعته إلى المشتري من وقت التسليم الفعلي.</li>
    </ol>
  </div>

  <div class="box">
    <div class="h">خامساً: ضمان الاستحقاق والتعرّض والعيوب الخفية</div>
    <ol class="ol">
      <li>يضمن البائع عدم التعرّض للمشتري في الانتفاع بالمبيع وخلوّه من حقوق الغير (ضمان الاستحقاق).</li>
      <li>يضمن البائع خلوّ المبيع من العيوب الخفية التي تنقص من قيمته أو من الانتفاع به وفق أحكام قانون المعاملات المدنية، ما لم يُتفق على الإعفاء من الضمان في حدود ما يجيزه القانون ودون أن يعلم البائع بالعيب ويكتمه.</li>
  ...
  <div class="box">
    ...
    <ol class="ol">
      ...
      <li>تختص المحكمة المختصة في <b>{{disputeCity}}</b> بنظر أي نزاع ينشأ عن هذا العقد ما لم يتفق الطرفان كتابةً على التحكيم أو على خلاف ذلك.</li>
    </ol>
  </div>
...
export const SALE_OM_EN: ContractTemplate = {
  id: 17102,
  slug: "om-sale-movable-en",
  title: "Movable Property Sale Agreement (Oman) — English",
  lang: "en",
  group: "PRO",
  jurisdiction: "OM",
  fields: [
    ...
    { key: "contractCity", label: "Place/City of Execution", required: true, type: "text", group: "Contract Info", placeholder: "Muscat" },

    { key: "sellerName", label: "Seller Name", required: true, type: "text", group: "Seller" },
    { key: "sellerId", label: "Seller Civil ID / CR", required: true, type: "text", group: "Seller" },
    ...
    { key: "buyerId", label: "Buyer Civil ID / CR", required: true, type: "text", group: "Buyer" },
    ...
    { key: "priceCurrency", label: "Currency", required: true, type: "select", group: "Price & Payment",
      options: currencyOptionsEn(OM.currencies) },
    ...
    { key: "governingLaw", label: "Governing Law", required: false, type: "text", group: "Provisions", placeholder: OM.governingLawEn },
    { key: "disputeCity", label: "Jurisdiction / Court", required: false, type: "text", group: "Provisions", placeholder: OM.defaultCourtCityEn },
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="muted">Governed by the Omani Civil Transactions Law (Royal Decree No. 29/2013).</div>
    </div>
    <div class="meta">
  ...
  <div class="box"><div class="sec"><h3>1. Parties</h3>
    <div class="row">
      <div><b>Seller:</b> {{sellerName}}<br/><b>Civil ID/CR:</b> {{sellerId}}<br/><b>Address:</b> {{sellerAddress}}<br/><b>Contact:</b> {{sellerPhone}}</div>
      <div><b>Buyer:</b> {{buyerName}}<br/><b>Civil ID/CR:</b> {{buyerId}}<br/><b>Address:</b> {{buyerAddress}}<br/><b>Contact:</b> {{buyerPhone}}</div>
    </div>
    <div class="muted">The Seller and Buyer are together the "Parties".</div>
  </div></div>
  ...
  <div class="sec"><h3>2. Subject Matter</h3><div class="box">
    ...
    <div class="muted">The Seller represents that it lawfully owns the item and that it is free of any third-party right unless expressly disclosed.</div>
  </div></div>
  ...
  <div class="sec"><h3>3. Price & Payment</h3><div class="box">
    ...
    <div class="muted">This Agreement is binding on the Parties; neither may unilaterally rescind or vary it save by mutual consent or on grounds provided by the Civil Transactions Law.</div>
  </div></div>
  ...
  <div class="sec"><h3>4. Delivery, Title & Risk</h3><div class="box">
    ...
    <div class="muted">Title passes to the Buyer unless otherwise agreed, and risk of loss passes to the Buyer upon actual delivery.</div>
  </div></div>

  <div class="sec"><h3>5. Warranty of Title & Latent Defects</h3><div class="box">
    <div><b>Warranty Terms (if any):</b> {{warrantyTerms}}</div>
    <div class="muted">The Seller warrants quiet possession against dispossession and third-party claims, and is liable for latent defects that reduce the item's value or fitness for use under the Civil Transactions Law, save for a lawful exclusion of liability where the Seller did not knowingly conceal the defect.</div>
  </div></div>
  ...
  <div class="sec"><h3>6. Governing Law & Dispute Resolution</h3><div class="box">
    ...
    <div class="muted">If left blank, the laws of the Sultanate of Oman apply and the competent court in {{contractCity}} has jurisdiction unless the Parties agree in writing to arbitration.</div>
