// lib/contracts/jo/services.ts
// عقود الخدمات الأردنية (استشارة، تدريب، تسويق، تطوير برمجي، خدمات إلكترونية، ترخيص، إدارة).
// تخضع للقواعد العامة للعقود في القانون المدني الأردني رقم 43 لسنة 1976 (القوة الملزمة
// للعقد وتنفيذه بحسن النية)، مع مراعاة القوانين الخاصة: حماية حق المؤلف رقم 22 لسنة 1992،
// المعاملات الإلكترونية رقم 15 لسنة 2015، حماية البيانات الشخصية رقم 24 لسنة 2023،
...
function buildServiceContract(s: ServiceSpec): {
  ...
  const arFields: ContractField[] = [
    ...
    { key: "providerId", label: "الرقم الوطني/السجل التجاري", required: true, type: "text", group: s.providerAr },
    ...
    { key: "clientId", label: "الرقم الوطني/السجل التجاري", required: true, type: "text", group: s.clientAr },
  ...
  <div class="box">
    ...
    <div class="clause">تخضع حقوق المؤلف والحقوق المجاورة والمصنفات (ومنها البرمجيات) لأحكام قانون حماية حق المؤلف الأردني رقم 22 لسنة 1992؛ ولا يجوز لأي طرف استعمال علامات أو مصنفات الطرف الآخر خارج حدود هذا العقد.</div>
  </div>`
    : "";
...
<div class="doc rtl">
  ...
  <div class="box">
    ...
    <div class="p"><b>${s.providerAr}:</b> {{providerName}} — الرقم الوطني/السجل: {{providerId}} — العنوان: {{providerAddress}}</div>
    <div class="p"><b>${s.clientAr}:</b> {{clientName}} — الرقم الوطني/السجل: {{clientId}} — العنوان: {{clientAddress}}</div>
  </div>
  ...
  <div class="box">
    ...
    <div class="clause">العقد ملزم للطرفين فلا يجوز نقضه ولا تعديله إلا باتفاقهما أو للأسباب التي يقرّرها القانون، ويجب تنفيذه بما يتفق مع حسن النية، عملاً بأحكام القانون المدني الأردني.</div>
  </div>
...
${arIpBlock}
  ...
  <div class="sec"><h3>5. Intellectual Property</h3><div class="box">
    <div>{{ipOwnership}}</div>
    <div class="muted">Copyright, neighbouring rights and works (including software) are subject to the Jordanian Copyright Protection Law No. 22 of 1992; neither party may use the other's marks or works beyond this contract.</div>
  </div></div>`
    : "";
  ...
  const EN: ContractTemplate = {
    id: s.idBase + 2,
    slug: `jo-${s.slug}-en`,
    title: `${s.titleEn} (Jordan) — English`,
    lang: "en",
    group: "PRO",
    jurisdiction: "JO",
    fields: enFields,
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="sec"><h3>2. Scope & Deliverables</h3><div class="box">
    ...
    <div class="muted">This Agreement is binding on the Parties and may be amended or rescinded only by mutual consent or on grounds provided by law, and must be performed in good faith under the Jordanian Civil Code.</div>
  </div></div>
...
const GENERAL_LAW_AR =
  "مصاغ وفق القواعد العامة للعقود في القانون المدني الأردني رقم 43 لسنة 1976";
const GENERAL_LAW_EN =
  "Governed by the general contract rules of the Jordanian Civil Code No. 43 of 1976";

const consultancy = buildServiceContract({
  idBase: 8300, slug: "consultancy",
...
const marketing = buildServiceContract({
  ...
  lawNoteAr:
    "مصاغ وفق القانون المدني الأردني رقم 43 لسنة 1976، ومع مراعاة قانون حماية المستهلك رقم 7 لسنة 2017",
  lawNoteEn:
    "Governed by the Jordanian Civil Code No. 43 of 1976, with regard to Consumer Protection Law No. 7 of 2017",
  withIp: true,
});

const software = buildServiceContract({
  idBase: 8600, slug: "software-development",
  ...
  lawNoteAr:
    "مصاغ وفق القانون المدني الأردني رقم 43 لسنة 1976، ومع مراعاة قانون حماية حق المؤلف رقم 22 لسنة 1992",
  lawNoteEn:
    "Governed by the Jordanian Civil Code No. 43 of 1976, with regard to Copyright Protection Law No. 22 of 1992",
  withIp: true,
});

const eservices = buildServiceContract({
  idBase: 8700, slug: "e-services",
  ...
  lawNoteAr:
    "مصاغ وفق القانون المدني الأردني رقم 43 لسنة 1976، ومع مراعاة قانون المعاملات الإلكترونية رقم 15 لسنة 2015 وقانون حماية البيانات الشخصية رقم 24 لسنة 2023",
  lawNoteEn:
    "Governed by the Jordanian Civil Code No. 43 of 1976, with regard to the Electronic Transactions Law No. 15 of 2015 and Personal Data Protection Law No. 24 of 2023",
  withIp: true,
});

const licensing = buildServiceContract({
  idBase: 8800, slug: "licensing",
