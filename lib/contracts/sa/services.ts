// lib/contracts/sa/services.ts
// عقود الخدمات السعودية (استشارة، تدريب، تسويق، تطوير برمجي، خدمات إلكترونية، ترخيص، إدارة).
// تخضع للقواعد العامة للعقود في نظام المعاملات المدنية (م/191 لسنة 1444هـ) — القوة الملزمة للعقد
// وتنفيذه بحسن النية — مع مراعاة الأنظمة الخاصة: نظام حماية حقوق المؤلف (م/41 لسنة 1424هـ) والهيئة
// السعودية للملكية الفكرية (SAIP)، نظام التعاملات الإلكترونية (م/18 لسنة 1428هـ)، نظام حماية البيانات
// الشخصية (م/19 لسنة 1443هـ)، نظام التجارة الإلكترونية (م/126 لسنة 1440هـ).
...
function buildServiceContract(s: ServiceSpec): {
  ...
  const arFields: ContractField[] = [
    ...
    { key: "providerId", label: "رقم الهوية/السجل التجاري", required: true, type: "text", group: s.providerAr },
    ...
    { key: "clientId", label: "رقم الهوية/السجل التجاري", required: true, type: "text", group: s.clientAr },
  ...
  <div class="box">
    ...
    <div class="clause">تخضع حقوق المؤلف والحقوق المجاورة والمصنفات (ومنها البرمجيات) لأحكام نظام حماية حقوق المؤلف السعودي (م/41 لسنة 1424هـ) تحت إشراف الهيئة السعودية للملكية الفكرية (SAIP)؛ ولا يجوز لأي طرف استعمال علامات أو مصنفات الطرف الآخر خارج حدود هذا العقد.</div>
  </div>`
    : "";
...
<div class="doc rtl">
  ...
  <div class="box">
    ...
    <div class="clause">العقد ملزم للطرفين فلا يجوز نقضه ولا تعديله إلا باتفاقهما أو للأسباب التي يقرّرها النظام، ويجب تنفيذه بما يتفق مع حسن النية، عملاً بأحكام نظام المعاملات المدنية.</div>
  </div>
...
${arIpBlock}
  ...
  <div class="box">
    <div class="h">${s.withIp ? "سابعاً" : "سادساً"}: النظام الواجب التطبيق وتسوية النزاعات</div>
    <ol class="ol">
  ...
  <div class="sec"><h3>5. Intellectual Property</h3><div class="box">
    <div>{{ipOwnership}}</div>
    <div class="muted">Copyright, neighbouring rights and works (including software) are subject to the Saudi Copyright Law (M/41 of 1424H), administered by the Saudi Authority for Intellectual Property (SAIP); neither party may use the other's marks or works beyond this contract.</div>
  </div></div>`
    : "";
  ...
  const EN: ContractTemplate = {
    id: s.idBase + 2,
    slug: `sa-${s.slug}-en`,
    title: `${s.titleEn} (Saudi Arabia) — English`,
    lang: "en",
    group: "PRO",
    jurisdiction: "SA",
    fields: enFields,
...
const GENERAL_LAW_AR =
  "مصاغ وفق القواعد العامة للعقود في نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ)";
const GENERAL_LAW_EN =
  "Governed by the general contract rules of the Saudi Civil Transactions Law (M/191 of 1444H)";

const consultancy = buildServiceContract({
  idBase: 12300, slug: "consultancy",
...
const marketing = buildServiceContract({
  ...
  lawNoteAr:
    "مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ)، ومع مراعاة نظام التجارة الإلكترونية (م/126 لسنة 1440هـ)",
  lawNoteEn:
    "Governed by the Saudi Civil Transactions Law (M/191 of 1444H), with regard to the E-Commerce Law (M/126 of 1440H)",
  withIp: true,
});

const software = buildServiceContract({
  idBase: 12600, slug: "software-development",
  ...
  lawNoteAr:
    "مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ)، ومع مراعاة نظام حماية حقوق المؤلف (م/41 لسنة 1424هـ)",
  lawNoteEn:
    "Governed by the Saudi Civil Transactions Law (M/191 of 1444H), with regard to the Copyright Law (M/41 of 1424H)",
  withIp: true,
});

const eservices = buildServiceContract({
  idBase: 12700, slug: "e-services",
  ...
  lawNoteAr:
    "مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ)، ومع مراعاة نظام التعاملات الإلكترونية (م/18 لسنة 1428هـ) ونظام حماية البيانات الشخصية (م/19 لسنة 1443هـ)",
  lawNoteEn:
    "Governed by the Saudi Civil Transactions Law (M/191 of 1444H), with regard to the Electronic Transactions Law (M/18 of 1428H) and the Personal Data Protection Law (M/19 of 1443H)",
  withIp: true,
});

const licensing = buildServiceContract({
  idBase: 12800, slug: "licensing",
