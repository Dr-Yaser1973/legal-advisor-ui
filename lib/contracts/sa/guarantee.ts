// lib/contracts/sa/guarantee.ts
// عقد كفالة وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام الكفالة.
...
export const GUARANTEE_SA_AR: ContractTemplate = {
  id: 11801,
  slug: "sa-guarantee-ar",
  title: "عقد كفالة (السعودية) – عربي",
  lang: "ar",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "creditorId", label: "رقم الهوية/السجل للدائن", required: true, type: "text", group: "الدائن" },
    ...
    { key: "debtorId", label: "رقم الهوية/السجل للمدين", required: true, type: "text", group: "المدين الأصلي" },
    ...
    { key: "guarantorId", label: "رقم الهوية/السجل للكفيل", required: true, type: "text", group: "الكفيل" },
    ...
    { key: "guaranteeScope", label: "نطاق الكفالة", required: true, type: "select", group: "محل الكفالة",
      options: ["أصل الدين وملحقاته (المصروفات وما يستحق نظاماً)", "أصل الدين فقط"] },
...
<div class="doc rtl">
  <div class="header">
    ...
    <div class="subtitle">مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام الكفالة</div>
    <div class="meta">
  ...
  <div class="box">
    ...
    <div class="p"><b>الدائن:</b> {{creditorName}} — الهوية/السجل: {{creditorId}} — العنوان: {{creditorAddress}}</div>
    <div class="p"><b>المدين الأصلي:</b> {{debtorName}} — الهوية/السجل: {{debtorId}} — العنوان: {{debtorAddress}}</div>
    <div class="p"><b>الكفيل:</b> {{guarantorName}} — الهوية/السجل: {{guarantorId}} — العنوان: {{guarantorAddress}}</div>
  </div>
  ...
  <div class="box">
    ...
    <div class="clause">يكفل الكفيل تنفيذ التزام المدين إذا لم يفِ به المدين نفسه، ولا تصحّ الكفالة إلا إذا كان الالتزام المكفول صحيحاً، عملاً بأحكام الكفالة في نظام المعاملات المدنية.</div>
  </div>
  ...
  <div class="box">
    <div class="h">رابعاً: الشروط الخاصة والنظام الواجب</div>
...
export const GUARANTEE_SA_EN: ContractTemplate = {
  id: 11802,
  slug: "sa-guarantee-en",
  title: "Suretyship / Guarantee (Saudi Arabia) — English",
  lang: "en",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "creditorId", label: "Creditor ID / CR", required: true, type: "text", group: "Creditor" },
    ...
    { key: "debtorId", label: "Debtor ID / CR", required: true, type: "text", group: "Principal Debtor" },
    ...
    { key: "guarantorId", label: "Guarantor ID / CR", required: true, type: "text", group: "Guarantor" },
    ...
    { key: "guaranteeScope", label: "Scope of Guarantee", required: true, type: "select", group: "Scope",
      options: ["Principal + accessories (costs & statutory sums)", "Principal only"] },
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="muted">Governed by the Saudi Civil Transactions Law (M/191 of 1444H) — Suretyship provisions.</div>
    </div>
    <div class="meta">
  ...
  <div class="box"><div class="sec"><h3>1. Parties</h3>
    <div><b>Creditor:</b> {{creditorName}} — ID/CR: {{creditorId}} — {{creditorAddress}}</div>
    <div><b>Principal Debtor:</b> {{debtorName}} — ID/CR: {{debtorId}} — {{debtorAddress}}</div>
    <div><b>Guarantor:</b> {{guarantorName}} — ID/CR: {{guarantorId}} — {{guarantorAddress}}</div>
  </div></div>
