// lib/contracts/sa/pledge.ts
// عقد رهن حيازي (ضمان دين) وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام الرهن الحيازي.
// وللرهن على المنقولات يُراعى نظام الرهن التجاري والسجل الموحّد للرهون على الأموال المنقولة.
...
export const PLEDGE_SA_AR: ContractTemplate = {
  id: 11701,
  slug: "sa-pledge-ar",
  title: "عقد رهن حيازي (السعودية) – عربي",
  lang: "ar",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "pledgorId", label: "رقم الهوية/السجل للراهن", required: true, type: "text", group: "الراهن" },
    ...
    { key: "pledgeeId", label: "رقم الهوية/السجل للمرتهن", required: true, type: "text", group: "المرتهن" },
...
<div class="doc rtl">
  <div class="header">
    ...
    <div class="subtitle">مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام الرهن الحيازي</div>
    <div class="meta">
  ...
  <div class="box">
    ...
    <div class="p"><b>الراهن:</b> {{pledgorName}} — الهوية/السجل: {{pledgorId}} — العنوان: {{pledgorAddress}}</div>
    <div class="p"><b>المرتهن (الدائن):</b> {{pledgeeName}} — الهوية/السجل: {{pledgeeId}} — العنوان: {{pledgeeAddress}}</div>
  </div>
  ...
  <div class="box">
    ...
    <div class="clause">لا يتمّ الرهن الحيازي إلا بتسليم المرهون إلى المرتهن أو إلى عدلٍ يتفق عليه الطرفان، ويبقى المرهون محبوساً ضماناً للدين وملحقاته، عملاً بأحكام الرهن الحيازي في نظام المعاملات المدنية.</div>
  </div>
  ...
  <div class="box">
    ...
    <ol class="ol">
      ...
      <li>يقع باطلاً كل اتفاق يجعل المرهون ملكاً للمرتهن عند عدم الوفاء، أو يبيح له بيعه دون مراعاة الإجراءات التي قرّرها النظام.</li>
    </ol>
  </div>

  <div class="box">
    <div class="h">خامساً: التنفيذ عند عدم الوفاء</div>
    <div class="p">إذا لم يُوفَّ الدين في ميعاده جاز للمرتهن التنفيذ على المرهون وبيعه بالمزاد وفق إجراءات نظام التنفيذ واستيفاء دينه من ثمنه بحق الأولوية، ويُردّ الفائض للراهن.</div>
  </div>

  <div class="box">
    <div class="h">سادساً: الشروط الخاصة والنظام الواجب</div>
    <div class="p"><b>شروط خاصة:</b> {{specialTerms}}</div>
    <ol class="ol">
      <li>يخضع هذا العقد ويُفسَّر وفق: <b>{{governingLaw}}</b> (ويُقيَّد الرهن على المنقولات في السجل الموحّد للرهون على الأموال المنقولة عند الاقتضاء لنفاذه في مواجهة الغير).</li>
...
export const PLEDGE_SA_EN: ContractTemplate = {
  id: 11702,
  slug: "sa-pledge-en",
  title: "Possessory Pledge Agreement (Saudi Arabia) — English",
  lang: "en",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "pledgorId", label: "Pledgor ID / CR", required: true, type: "text", group: "Pledgor" },
    ...
    { key: "pledgeeId", label: "Pledgee ID / CR", required: true, type: "text", group: "Pledgee" },
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="muted">Governed by the Saudi Civil Transactions Law (M/191 of 1444H) — Possessory Pledge provisions.</div>
    </div>
    <div class="meta">
  ...
  <div class="box"><div class="sec"><h3>1. Parties</h3>
    <div><b>Pledgor:</b> {{pledgorName}} — ID/CR: {{pledgorId}} — {{pledgorAddress}}</div>
    <div><b>Pledgee (Creditor):</b> {{pledgeeName}} — ID/CR: {{pledgeeId}} — {{pledgeeAddress}}</div>
  </div></div>
  ...
  <div class="sec"><h3>5. Enforcement on Default</h3><div class="box">
    <div class="muted">On default, the Pledgee may enforce against the asset and sell it by public auction under the Enforcement Law, recover the debt from the proceeds by priority, and return any surplus to the Pledgor.</div>
  </div></div>
  ...
  <div class="sec"><h3>6. Governing Law & Special Terms</h3><div class="box">
    ...
    <div class="muted">A pledge over movable assets is, where applicable, registered in the Unified Register for Movable Pledges to be effective against third parties.</div>
  </div></div>
