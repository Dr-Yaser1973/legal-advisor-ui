// lib/contracts/sa/loan.ts
// عقد قرض وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام القرض.
// القرض المدني بين الأفراد يُردّ بمثل مقداره دون زيادة؛ ولا تجري أعمال التمويل بمقابل
// إلا عبر جهة تمويل مرخّصة من البنك المركزي السعودي (ساما) وفق نظام مراقبة شركات التمويل.
...
export const LOAN_SA_AR: ContractTemplate = {
  id: 11901,
  slug: "sa-loan-ar",
  title: "عقد قرض (السعودية) – عربي",
  lang: "ar",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "lenderId", label: "رقم الهوية/السجل للمُقرِض", required: true, type: "text", group: "المُقرِض" },
    ...
    { key: "borrowerId", label: "رقم الهوية/السجل للمُقترِض", required: true, type: "text", group: "المُقترِض" },
    ...
    { key: "repaymentType", label: "طريقة السداد", required: true, type: "select", group: "السداد",
      options: ["دفعة واحدة في تاريخ الاستحقاق", "أقساط دورية"] },
    { key: "repaymentSchedule", label: "جدول السداد/الأقساط", required: true, type: "textarea", group: "السداد" },
    { key: "dueDate", label: "تاريخ استحقاق السداد النهائي", required: true, type: "date", group: "السداد" },
    { key: "financingNote", label: "شروط التمويل (إن كان المُقرِض جهة تمويل مرخّصة من ساما)", required: false, type: "textarea", group: "السداد",
      placeholder: "تُذكر شروط التمويل ونسبته فقط إذا كان المُقرِض شركة تمويل مرخّصة" },
...
<div class="doc rtl">
  <div class="header">
    ...
    <div class="subtitle">مصاغ وفق نظام المعاملات المدنية السعودي (م/191 لسنة 1444هـ) — أحكام القرض</div>
    <div class="meta">
  ...
  <div class="box">
    ...
    <div class="p"><b>المُقرِض:</b> {{lenderName}} — الهوية/السجل: {{lenderId}} — العنوان: {{lenderAddress}}</div>
    <div class="p"><b>المُقترِض:</b> {{borrowerName}} — الهوية/السجل: {{borrowerId}} — العنوان: {{borrowerAddress}}</div>
  </div>
  ...
  <div class="box">
    ...
    <div class="clause">يُملّك المُقرِض المُقترِض مبلغ القرض المبيّن أعلاه، ويقرّ المُقترِض باستلامه، ويلتزم بردّ مثله جنساً وقدراً وصفةً في ميعاد الاستحقاق، عملاً بأحكام القرض في نظام المعاملات المدنية.</div>
  </div>

  <div class="box">
    <div class="h">ثالثاً: السداد</div>
    ...
    <div class="clause">يُردّ القرض بمثل مقداره دون زيادة على الأصل بين غير جهات التمويل المرخّصة. وإذا كان المُقرِض شركة تمويل مرخّصة من البنك المركزي السعودي (ساما) سرت شروط التمويل ونسبته وفق نظام مراقبة شركات التمويل ولوائحه: {{financingNote}}</div>
  </div>
  ...
  <div class="box">
    <div class="h">خامساً: النظام الواجب التطبيق وتسوية النزاعات</div>
    <ol class="ol">
...
export const LOAN_SA_EN: ContractTemplate = {
  id: 11902,
  slug: "sa-loan-en",
  title: "Loan Agreement (Saudi Arabia) — English",
  lang: "en",
  group: "PRO",
  jurisdiction: "SA",
  fields: [
    ...
    { key: "lenderId", label: "Lender ID / CR", required: true, type: "text", group: "Lender" },
    ...
    { key: "borrowerId", label: "Borrower ID / CR", required: true, type: "text", group: "Borrower" },
    ...
    { key: "repaymentType", label: "Repayment", required: true, type: "select", group: "Repayment",
      options: ["Lump sum at maturity", "Periodic instalments"] },
    { key: "repaymentSchedule", label: "Repayment Schedule", required: true, type: "textarea", group: "Repayment" },
    { key: "dueDate", label: "Final Due Date", required: true, type: "date", group: "Repayment" },
    { key: "financingNote", label: "Financing terms (only if the Lender is a SAMA-licensed financier)", required: false, type: "textarea", group: "Repayment" },
...
<div class="doc" dir="ltr" lang="en">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="muted">Governed by the Saudi Civil Transactions Law (M/191 of 1444H) — Loan provisions.</div>
    </div>
    <div class="meta">
  ...
  <div class="sec"><h3>2. Loan Amount & Disbursement</h3><div class="box">
    ...
    <div class="muted">The Lender transfers ownership of the loan amount to the Borrower, who acknowledges receipt and undertakes to repay the like in kind, quantity and quality at maturity under the Civil Transactions Law.</div>
  </div></div>

  <div class="sec"><h3>3. Repayment</h3><div class="box">
    <div><b>Repayment:</b> {{repaymentType}}</div>
    <div><b>Schedule:</b> {{repaymentSchedule}}</div>
    <div><b>Final Due Date:</b> {{dueDate}}</div>
    <div class="muted">Between parties other than licensed financiers, the loan is repaid in its principal amount without any increase. Where the Lender is a financing entity licensed by the Saudi Central Bank (SAMA), the financing terms and charge apply under the Finance Companies Control Law and its regulations: {{financingNote}}</div>
  </div></div>
