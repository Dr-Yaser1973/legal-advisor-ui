...
// ─────────────────────────────────────────
//  CFR — Cost and Freight (عربي + إنجليزي)
// ─────────────────────────────────────────
...
export const CFR_AR: ContractTemplate = {
  ...
  fields: [
    ...
    { key: "freightDetails", label: "تفاصيل أجرة الشحن", required: false, type: "textarea", group: "التسليم والشحن",
      placeholder: "شركة الشحن، نوع العقد (رحلة/وقت)" },

    // ── المستندات والفحص ──
    { key: "documentsList", label: "المستندات المطلوبة", required: true, type: "textarea", group: "المستندات والفحص",
      placeholder: "الفاتورة، بوليصة الشحن البحري، شهادة المنشأ" },
...
<div class="box">
  ...
  <p>
    يُقصد بشرط <b>CFR (التكلفة والشحن)</b> ما ورد تعريفه في
    <b>قواعد Incoterms® {{incotermsEdition}}</b> الصادرة عن غرفة التجارة الدولية (ICC).
  </p>
  <p class="small">
    في CFR يتحمّل البائع تكلفة الشحن البحري حتى ميناء الوصول، لكن الخطر ينتقل إلى المشتري
    عند وضع البضاعة على متن السفينة في ميناء الشحن — لا عند الوصول.
    التأمين البحري مسؤولية المشتري.
  </p>
</div>
...
<div class="box">
  ...
  <table>
    <tr>
      <th>وصف البضاعة</th><th>رمز HS</th><th>الكمية</th><th>الوحدة</th><th>نسبة التسامح</th>
    </tr>
    <tr>
      <td>{{goodsDescription}}</td><td>{{hsCode}}</td><td>{{quantity}}</td><td>{{unit}}</td><td>{{tolerance}}</td>
    </tr>
  </table>
  <table>
...
<div class="box">
  <h2>رابعاً: التسليم والشحن وفق CFR</h2>
  <table>
  ...
  <ul>
    <li>يلتزم البائع بإبرام عقد شحن بحري حتى ميناء الوصول المتفق عليه ودفع أجرة الشحن.</li>
    <li>تنتقل المخاطر من البائع إلى المشتري عند وضع البضاعة على متن السفينة في ميناء الشحن.</li>
...
<div class="box">
  ...
  <ul>
    <li>تنتقل مخاطر الهلاك أو التلف إلى المشتري عند شحن البضاعة على متن السفينة في <b>{{portOfShipment}}</b>.</li>
    <li>لا يلتزم البائع بالتأمين البحري في CFR — يتحمّل المشتري وحده مسؤولية تأمين البضاعة من لحظة الشحن.</li>
    <li>يُنصح المشتري بإبرام وثيقة تأمين بحري تغطي الرحلة من ميناء الشحن حتى الوجهة النهائية.</li>
  </ul>
</div>
...
export const CFR_EN: ContractTemplate = {
  ...
  fields: [
    ...
    { key: "freightDetails", label: "Freight Details", required: false, type: "textarea", group: "Delivery & Shipment",
      placeholder: "Carrier name, voyage/time charter details" },
    { key: "documentsList", label: "Required Documents", required: true, type: "textarea", group: "Documents & Inspection",
      placeholder: "Commercial invoice, ocean B/L, certificate of origin" },
...
<div class="box">
  ...
  <p>
    The term <b>CFR (Cost and Freight)</b> shall be interpreted in accordance with
    <b>Incoterms® {{incotermsEdition}}</b> issued by the ICC.
  </p>
  <p class="small">
    Under CFR, the Seller pays freight to the named port of destination, but risk transfers to the Buyer
    when the Goods are placed on board the vessel at the port of shipment — not upon arrival.
    Marine insurance is the Buyer's responsibility.
  </p>
</div>
...
<div class="box">
  ...
  <ul>
    <li>Seller shall contract for carriage by sea to the named port of destination and pay freight.</li>
    <li>Risk transfers to Buyer when Goods are placed on board the vessel at the port of shipment.</li>
...
<div class="box">
  ...
  <ul>
    <li>Risk passes from Seller to Buyer upon loading on board at <b>{{portOfShipment}}</b>.</li>
    <li>Seller has no obligation to procure marine insurance under CFR. Buyer is solely responsible for insuring the Goods from the moment of shipment.</li>
    <li>Buyer is strongly advised to obtain marine insurance covering the voyage to the final destination.</li>
  </ul>
</div>
