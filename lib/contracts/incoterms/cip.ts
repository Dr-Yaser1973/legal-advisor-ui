...
export const CIP_AR: ContractTemplate = {
  ...
  fields: [
    ...
    // CIP delivery fields
    { key: "placeOfHandover", label: "مكان تسليم البضاعة للناقل", required: true, type: "text", group: "التسليم والنقل",
      placeholder: "مستودع البائع، محطة الشحن..." },
    { key: "namedDestination", label: "مكان الوصول المحدد", required: true, type: "text", group: "التسليم والنقل",
      placeholder: "المستودع النهائي، المطار، الميناء الجاف..." },
    { key: "deliverySchedule", label: "جدول التسليم/الشحن", required: true, type: "text", group: "التسليم والنقل" },
    { key: "modeOfTransport", label: "وسيلة النقل", required: false, type: "select", group: "التسليم والنقل",
      options: ["بري", "بحري", "جوي", "متعدد الوسائط"] },
    { key: "carrierDetails", label: "تفاصيل الناقل", required: false, type: "textarea", group: "التسليم والنقل",
      placeholder: "اسم شركة الشحن، رقم التعاقد" },
    { key: "freightPaidTo", label: "أجرة النقل مدفوعة حتى", required: true, type: "text", group: "التسليم والنقل",
      placeholder: "يجب أن يطابق مكان الوصول المحدد" },
    // CIP-specific insurance (أعلى معيار من CIF)
    { key: "insuranceCoverage", label: "تغطية التأمين", required: true, type: "textarea", group: "التأمين",
      placeholder: "الحد الأدنى في CIP: شروط A (ICC Clause A) — أشمل تغطية" },
    ...
    { key: "documentsList", label: "المستندات المطلوبة", required: true, type: "textarea", group: "المستندات والفحص",
      placeholder: "الفاتورة، وثيقة النقل، وثيقة/شهادة التأمين، شهادة المنشأ" },
...
<div class="box">
  ...
  <p>يُقصد بشرط <b>CIP (النقل والتأمين مدفوعان حتى)</b> ما ورد تعريفه في <b>قواعد Incoterms® {{incotermsEdition}}</b> الصادرة عن ICC.</p>
  <p class="small">
    CIP = CPT + التزام البائع بالتأمين. الفارق الجوهري عن CIF: معيار التأمين في CIP أعلى —
    <b>ICC Clause A (شروط أ) كحد أدنى</b>، وهي أشمل تغطية مقارنة بـ ICC Clause C المطلوبة في CIF.
    مناسب لجميع وسائط النقل.
  </p>
</div>
...
<div class="box">
  <h2>رابعاً: التسليم والنقل وفق CIP</h2>
  <table>
    <tr><th style="width:35%">مكان تسليم البضاعة للناقل</th><td><b>{{placeOfHandover}}</b></td></tr>
    <tr><th>مكان الوصول المحدد</th><td><b>{{namedDestination}}</b></td></tr>
    <tr><th>جدول التسليم/الشحن</th><td>{{deliverySchedule}}</td></tr>
    <tr><th>وسيلة النقل</th><td>{{modeOfTransport}}</td></tr>
    <tr><th>تفاصيل الناقل</th><td>{{carrierDetails}}</td></tr>
    <tr><th>أجرة النقل مدفوعة حتى</th><td>{{freightPaidTo}}</td></tr>
  </table>
  <ul>
    <li>يلتزم البائع بإبرام عقد نقل حتى مكان الوصول المحدد ودفع أجرة النقل.</li>
    <li>تنتقل المخاطر إلى المشتري عند تسليم البضاعة للناقل الأول في <b>{{placeOfHandover}}</b>.</li>
    <li>يلتزم البائع بإنهاء إجراءات التصدير الجمركي.</li>
  </ul>
</div>

<div class="insurance-box">
  <h2>خامساً: التأمين (التزام البائع — معيار أعلى من CIF)</h2>
  <table>
  ...
  <ul>
    <li>يلتزم البائع بإبرام وثيقة تأمين لصالح المشتري بالحد الأدنى <b>ICC Clause A (شروط أ)</b> — وهي أشمل تغطية متاحة.</li>
    <li>تُسلَّم وثيقة التأمين أو شهادتها للمشتري ضمن حزمة المستندات.</li>
    <li>إذا اتفق الطرفان على تغطية أقل (ICC Clause B أو C)، يجب النص على ذلك صراحةً كتابةً.</li>
  </ul>
</div>
...
export const CIP_EN: ContractTemplate = {
  ...
  fields: [
    ...
    { key: "placeOfHandover", label: "Place of Handover to Carrier", required: true, type: "text", group: "Delivery & Transport",
      placeholder: "Seller's warehouse, freight terminal..." },
    { key: "namedDestination", label: "Named Place of Destination", required: true, type: "text", group: "Delivery & Transport",
      placeholder: "Final warehouse, dry port, airport..." },
    { key: "deliverySchedule", label: "Delivery/Shipment Schedule", required: true, type: "text", group: "Delivery & Transport" },
    { key: "modeOfTransport", label: "Mode of Transport", required: false, type: "select", group: "Delivery & Transport",
      options: ["Road", "Sea", "Air", "Multimodal"] },
    { key: "carrierDetails", label: "Carrier Details", required: false, type: "textarea", group: "Delivery & Transport",
      placeholder: "Carrier name, contract reference" },
    { key: "freightPaidTo", label: "Freight Paid To", required: true, type: "text", group: "Delivery & Transport",
      placeholder: "Must match the named place of destination" },
    // CIP-specific insurance (higher standard than CIF)
    { key: "insuranceCoverage", label: "Insurance Coverage", required: true, type: "textarea", group: "Insurance",
      placeholder: "CIP minimum: ICC (Cargo) Clause A — broadest cover" },
    ...
    { key: "documentsList", label: "Required Documents", required: true, type: "textarea", group: "Documents & Inspection",
      placeholder: "Commercial invoice, transport document, insurance policy/certificate, certificate of origin" },
...
<div class="box">
  ...
  <p>The term <b>CIP (Carriage and Insurance Paid To)</b> shall be interpreted in accordance with <b>Incoterms® {{incotermsEdition}}</b> issued by the ICC.</p>
  <p class="small">
    CIP = CPT + Seller's obligation to procure insurance. Key distinction from CIF: CIP requires
    <b>ICC (Cargo) Clause A as minimum</b> — the broadest cover available — whereas CIF only requires
    Clause C. Suitable for all modes of transport.
  </p>
</div>
...
<div class="box">
  <h2>4. Delivery and Transport – CIP</h2>
  <table>
    <tr><th style="width:35%">Place of Handover to Carrier</th><td><b>{{placeOfHandover}}</b></td></tr>
    <tr><th>Named Place of Destination</th><td><b>{{namedDestination}}</b></td></tr>
    <tr><th>Delivery/Shipment Schedule</th><td>{{deliverySchedule}}</td></tr>
    <tr><th>Mode of Transport</th><td>{{modeOfTransport}}</td></tr>
    <tr><th>Carrier Details</th><td>{{carrierDetails}}</td></tr>
    <tr><th>Freight Paid To</th><td>{{freightPaidTo}}</td></tr>
  </table>
  <ul>
    <li>Seller shall contract for carriage to the named place of destination and pay freight.</li>
    <li>Risk transfers to Buyer when Goods are handed to the first carrier at <b>{{placeOfHandover}}</b>.</li>
    <li>Seller shall complete all export customs formalities.</li>
  </ul>
</div>

<div class="insurance-box">
  <h2>5. Insurance (Seller's Obligation — Higher Standard than CIF)</h2>
  <table>
  ...
  <ul>
    <li>Seller shall procure cargo insurance in favour of the Buyer at minimum <b>ICC (Cargo) Clause A</b> — the broadest standard cover.</li>
    <li>The insurance policy or certificate shall be delivered to Buyer as part of the shipping documents.</li>
    <li>If the Parties agree to a lower cover (Clause B or C), this must be expressly stated in writing.</li>
  </ul>
</div>
